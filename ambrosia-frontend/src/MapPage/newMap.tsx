...
const useNewMap = ({ setFeatures }: { setFeatures: any }) => {
  ...
  const onUpdate = useCallback(async (e: any) => {
    setFeatures((currFeatures: any[]) => {
      ...
      const newFeatures: any = currFeatures.filter(
        (zone) =>
          !e.features.map((feature: any) => feature.id).includes(zone.id),
      );
  ...
  const handleMapLoad = async (lng: number, lat: number) => {
    ...
    mapboxgl.accessToken =
      "pk.eyJ1IjoiYWxleDg4MTIxMyIsImEiOiJjbHBkMTBmY2kwdmRkMmpxdDhwZ2kzN2J6In0.iTBF38aHM4k7CqWeqV3kiQ";
